...
@section('content')
    ...
    <style>
        .page-banner{
            background: url("{{ asset('website_style/images/8.jpg') }}") no-repeat fixed !important;
        }
    </style>
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12 text-center">
                    <h1 class="text-uppercase">{{ __('admin.resale') }}</h1>
                </div>
            </div>
        </div>
    </section>
                    ...
                    <div class="row">
                                ...
                                <div class="property_item heading_space">
                                    ...
                                    <div class="proerty_content">
                                        <div class="proerty_text">
                                            <h3 class="captlize"><a href="{{ url('resale/'.slug($property->{app()->getLocale().'_title'}).'-'.$property->id) }}">{{ $property->{app()->getLocale().'_title'} }}</a></h3>
                                            <p>{{ @App\Location::find($property->location_id)->{app()->getLocale().'_name'} }}</p>
                                        </div>
                                        @if(!auth()->guard('lead')->guest())

                                            <div class="fav like-btn-box" type="resale"  unit_id="{{ $property->id }}" style="cursor: pointer"><span>
                        ...
                        @foreach($all_projects as $property)
                            ...
                            <div class="col-sm-6 hidden to-filter"
                                ...
                                <div class="property_item heading_space">
                                    <div class="image">
                                        <a href="{{ url('resale/'.slug($property->{app()->getLocale().'_title'}).'-'.$property->id) }}">
                                            <img src="{{ url($property->watermarked_image) }}" alt="{{ $property->{app()->getLocale().'_name'} }}" class="img-responsive"></a>
                                        <div class="price clearfix">
                                            <span class="tag pull-right">{{ $property->price }} {{ __('admin.egp') }}</span>
                                        </div>
                                        <span class="tag_t">{{ __('admin.resale') }}</span>
                                    ...
                                    <div class="proerty_content">
                                        <div class="proerty_text">
                                            <h3 class="captlize"><a href="{{ url('resale/'.slug($property->{app()->getLocale().'_title'}).'-'.$property->id) }}">{{ $property->{app()->getLocale().'_title'} }}</a></h3>
                                            <p>{{ @App\Location::find($property->location)->{app()->getLocale().'_name'} }}</p>
                                        </div>
                                        @if(!auth()->guard('lead')->guest())
                                        ...
                                        <div class="property_meta transparent" >
                                            @foreach(@App\UnitFacility::where('unit_id',$property->id)->where('type','resale')->limit(3)->get() as $facility)
                ...
                <aside class="col-md-4 col-xs-12 ">
                    <div class="property-query-area clearfix">
                        <form class="callus" action="{{ url('search') }}" method="post" style="padding: 20px">
                            ...
                            <div class="row">
                                <input type="hidden" name="min_price" id="min_price" value="{{ $search['data']['resale_commercial_min_price'] }}">
                                <input type="hidden" name="max_price" id="max_price" value="{{ $search['data']['resale_commercial_max_price'] }}">
                                <input type="hidden" name="min_area" id="min_area" value="{{ $search['data']['resale_commercial_min_area'] }}">
                                <input type="hidden" name="max_area" id="max_area" value="{{ $search['data']['resale_commercial_max_area'] }}">
                                <div class="col-md-12 col-sm-12 col-xs-12 bottom15">
                                    <div class="single-query-slider">
                                        ...
                                        <div id="price_range" data-range_min="{{ $search['data']['resale_commercial_min_price'] }}"
                                             data-range_max="{{ $search['data']['resale_commercial_max_price'] }}"
                                             data-cur_min="{{ $search['data']['resale_commercial_min_price'] }}"
                                             data-cur_max="{{ $search['data']['resale_commercial_max_price'] }}"
                                             class="nstSlider">
                                            <div class="bar"></div>
                            ...
                            <div class="row">
                                <div class="col-md-12 col-sm-12 col-xs-12 bottom15">
                                    <div class="single-query-slider">
                                        ...
                                        <div id="area_range" data-range_min="{{ $search['data']['resale_commercial_min_area'] }}"
                                             data-range_max="{{ $search['data']['resale_commercial_max_area'] }}"
                                             data-cur_min="{{ $search['data']['resale_commercial_min_area'] }}"
                                             data-cur_max="{{ $search['data']['resale_commercial_max_area'] }}"
                                             class="nstSlider">
                                            <div class="bar"></div>
                                            <div class="leftGrip"></div>
                                            <div class="rightGrip"></div>
...
@section('js')
    <script>
        ...
        $(document).ready(function () {
            $('#price_range').nstSlider('set_range', parseInt("{{ $search['data']['resale_commercial_min_price'] }}"), parseInt("{{ $search['data']['resale_commercial_max_price'] }}"));
            $('#area_range').nstSlider('set_range', parseInt("{{ $search['data']['resale_commercial_min_area'] }}"), parseInt("{{ $search['data']['resale_commercial_max_area'] }}"));
            $('#price_range').nstSlider('set_position', parseInt("{{ $search['data']['resale_commercial_min_price'] }}"), parseInt("{{ $search['data']['resale_commercial_max_price'] }}"));
            $('#area_range').nstSlider('set_position', parseInt("{{ $search['data']['resale_commercial_min_area'] }}"), parseInt("{{ $search['data']['resale_commercial_max_area'] }}"));
