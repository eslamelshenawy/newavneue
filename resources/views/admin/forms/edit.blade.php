...
@section('content')
    <div class="box">
        ...
        <div class="box-body">
            {!! Form::open(['url' => adminPath().'/forms/'.$form->id , 'method'=>'put','enctype'=>'multipart/form-data']) !!}
            <div class="row">
                <div class="form-group @if($errors->has('type')) has-error @endif col-md-12">
                    ...
                    <select class="form-control select2" name="type" id="type"
                        ...
                        <option></option>
                        <option @if($form->type == 'project') selected
                                @endif value="project">{{ __('admin.project') }}</option>
                        <option @if($form->type == 'event') selected
                                @endif value="event">{{ __('admin.event') }}</option>
                        <option @if($form->type == 'campaign') selected
                                @endif value="campaign">{{ __('admin.campaign') }}</option>
                    </select>
                </div>
            </div>
            ...
            <div class="row">
                <div class="form-group @if($errors->has('ar_title')) has-error @endif col-md-6">
                    ...
                    {!! Form::text('ar_title',$form->ar_title,['class' => 'form-control', 'placeholder' => trans('admin.ar_title')]) !!}
                </div>
                <div class="form-group @if($errors->has('en_title')) has-error @endif col-md-6">
                    <label>{{ trans('admin.en_title') }}</label>
                    {!! Form::text('en_title',$form->en_title,['class' => 'form-control', 'placeholder' => trans('admin.en_title')]) !!}
                </div>
            </div>
                ...
                <div class="form-group @if($errors->has('lead_source_id')) has-error @endif col-md-6">
                    ...
                    <select class="form-control select2" name="lead_source_id"
                        ...
                        @foreach(@\App\LeadSource::get() as $src)
                            <option @if($form->lead_source_id == $src->id) selected
                                    @endif value="{{ $src->id }}">{{ $src->name }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            @php($selecteds = json_decode($form->fields))
            @php($i = 0)

            @foreach($selecteds as $selected => $v)
                <div class="row" id="fieldo{{ $i }}">
                    <div class="form-group @if($errors->has('fields')) has-error @endif col-md-9">
                        <label>{{ trans('admin.fields') }}</label>
                        <select class="form-control select2" name="fields[o{{ $i }}]"
                                data-placeholder="{{ __('admin.fields') }}">
                            <option></option>
                            @foreach($fields as $field)
                                <option @if($selected == $field) selected
                                        @endif value="{{ $field }}">{{ __('admin.'.$field) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-2">
                        <br/>
                        <input type="hidden" name="required[o{{ $i }}]" value="0">
                        <input type="checkbox" name="required[o{{ $i }}]" class="switch-box"
                    ...
                    <div class="col-md-1">
                        <br/>
                        @if($loop->first)
                            <i class="fa fa-plus" id="addField"
                               style="font-size: 1.8em; cursor: pointer; padding-top: 10px"></i>
                        @else
                            <i class="fa fa-minus removeField" count="o{{ $i }}"
                               style="font-size: 1.8em; cursor: pointer; padding-top: 10px"></i>
                        @endif
                    </div>
                </div>
            ...
            <div class="well" @if($form->type != 'project') style="display: none" @endif id="projects">
                <div class="form-group @if($errors->has('developer_id')) has-error @endif">
                    ...
                    <select class="form-control select2" name="developer_id" id="developer_id"
                        ...
                        @foreach(@\App\Developer::get() as $dev)
                            <option value="{{ $dev->id }}" @if($form->developer_id == $dev->id) selected @endif>
                ...
                <div id="getProjects">
                    <div class="form-group @if($errors->has('project_id')) has-error @endif">
                        ...
                        <select class="form-control select2" name="project_id" id="project_id"
                                data-placeholder="{{ __('admin.project') }}" style="width: 100%">
                            <option></option>
                            @foreach(@\App\Project::where('developer_id', $form->developer_id)->get() as $pro)
                                <option value="{{ $pro->id }}" @if($form->project_id == $pro->id) selected @endif>
                    ...
                    <div id="getPhases">
                        <div class="form-group @if($errors->has('phase_id')) has-error @endif">
                            ...
                            <select class="form-control select2" name="phase_id" id="phase_id"
                                ...
                                <option></option>
                                @foreach(@\App\Phase::where('project_id', $form->project_id)->get() as $phase)
                                    <option value="{{ $phase->id }}" @if($form->phase_id == $phase->id) selected @endif>
            ...
            <div class="well" @if($form->type != 'event') style="display: none" @endif id="events">
                <div class="form-group @if($errors->has('event_id')) has-error @endif">
                    ...
                    <select class="form-control select2" name="event_id" id="event_id"
                        ...
                        @foreach(@\App\Event::get() as $evnet)
                            <option value="{{ $evnet->id }}" @if($form->event_id == $evnet->id) selected @endif>
            ...
            <div class="well" @if($form->type != 'campaign') style="display: none" @endif id="campaigns">
                <div class="form-group @if($errors->has('campaign_id')) has-error @endif">
                    ...
                    <select class="form-control select2" name="campaign_id" id="campaign_id"
                        ...
                        <option></option>
                        @foreach(@\App\Campaign::get() as $campaign)
                            <option value="{{ $campaign->id }}"
                                    @if($form->campaign_id == $campaign->id) selected @endif>
                                {{ $campaign->{app()->getLocale().'_title'} }}
                            </option>
                        @endforeach
                    </select>
                </div>
