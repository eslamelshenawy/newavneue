    ...
    <div class="box">
        ...
        <div class="box-body">
            <a class="btn btn-success btn-flat @if(app()->getLocale() == 'en') pull-right @else pull-left @endif"
               href="{{ url(adminPath().'/industries/create') }}">{{ trans('admin.add') }}</a>
            <table class="table table-hover table-striped datatable">
                ...
                @foreach($industries as $industry)
                    <tr>
                        <td>{{ $industry->id }}</td>
                        <td>{{ $industry->name }}</td>
                        <td><a href="{{ url(adminPath().'/industries/'.$industry->id) }}"
                               class="btn btn-primary btn-flat">{{ trans('admin.show') }}</a></td>
                        <td><a href="{{ url(adminPath().'/industries/'.$industry->id.'/edit') }}"
                               class="btn btn-warning btn-flat">{{ trans('admin.edit') }}</a></td>
                        <td><a data-toggle="modal" data-target="#delete{{ $industry->id }}"
                               class="btn btn-danger btn-flat">{{ trans('admin.delete') }}</a></td>
                    </tr>
                    <div id="delete{{ $industry->id }}" class="modal fade" role="dialog">
                        <div class="modal-dialog">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <p>{{ trans('admin.delete') . ' ' . $industry->name }}</p>
                                </div>
                                <div class="modal-footer">
                                    {!! Form::open(['method'=>'DELETE','route'=>['industries.destroy',$industry->id]]) !!}
