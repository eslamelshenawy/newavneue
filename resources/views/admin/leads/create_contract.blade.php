...
@section('content')
    <div class="box">
        ...
        <div class="box-body">
            <form action="{{ url(adminPath().'/contracts') }}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}

                <div class="form-group @if($errors->has('title')) has-error @endif @if(request()->has('lead')) col-md-12 @else col-md-6 @endif">
                    <label>{{ trans('admin.title') }}</label>
                    <input type="text" class="form-control" name="title" placeholder="{{ __('admin.title') }}">
                </div>
                ...
                <div class="form-group @if($errors->has('lead_id')) has-error @endif col-md-6 @if(request()->has('lead')) hidden @endif">
                    ...
                    <select name="lead_id" class="form-control select2" style="width: 100%"
                        ...
                        @foreach(@\App\Lead::getAgentLeads() as $lead)
                            <option @if(request()->lead == $lead->id) selected @endif value="{{ $lead->id }}">{{ $lead->first_name . ' ' . $lead->last_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group @if($errors->has('contract')) has-error @endif col-md-12">
                    ...
                    <textarea id="editor1" name="contract" rows="10" cols="80"></textarea>
                </div>
