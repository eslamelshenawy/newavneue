    ...
    <div class="box">
        ...
        <div class="box-body">
            <a class="btn btn-success btn-flat @if(app()->getLocale() == 'en') pull-right @else pull-left @endif"
               href="{{ url(adminPath().'/unit_types/create') }}">{{ trans('admin.add') }}</a>
            <table class="table table-hover table-striped datatable">
                ...
                @foreach($index as $src)
                    <tr>
                        ...
                        <td>{{ $src->en_name }}</td>
                        <td>{{ $src->ar_name }}</td>
                        <td><a href="{{ url(adminPath().'/unit_types/'.$src->id) }}"
                               class="btn btn-primary btn-flat">{{ trans('admin.show') }}</a></td>
                        <td><a href="{{ url(adminPath().'/unit_types/'.$src->id.'/edit') }}"
                               class="btn btn-warning btn-flat">{{ trans('admin.edit') }}</a></td>
                        <td><a data-toggle="modal" data-target="#delete"
                        ...
                        <div class="modal-dialog">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <p>{{ trans('admin.delete') . ' ' . $src->en_name }}</p>
                                </div>
                                <div class="modal-footer">
                                    {!! Form::open(['method'=>'DELETE','route'=>['unit_types.destroy',$src->id]]) !!}
