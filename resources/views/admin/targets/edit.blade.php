...
@section('content')
    <div class="box">
        ...
        <div class="box-body">
            {!! Form::open(['url' => adminPath().'/targets/'.$target->id , 'method'=>'put']) !!}
            <div class="form-group @if($errors->has('agent_type_id')) has-error @endif">
                ...
                <select class="form-control select2"  name="agent_type_id" data-placeholder="{{ trans('admin.agent_type') }}">
                    ...
                    @foreach(@App\AgentType::get() as $type)
                        <option value="{{ $type->id }}" @if($type->id == $target->agent_type_id) selected @endif>{{ $type->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group @if($errors->has('calls')) has-error @endif">
                <label>{{ trans('admin.calls') }}</label>
                {!! Form::number('calls',$target->calls,['class' => 'form-control', 'placeholder' => trans('admin.calls'),'min'=>0]) !!}
            </div>
            <div class="form-group @if($errors->has('meetings')) has-error @endif">
                <label>{{ trans('admin.meetings') }}</label>
                {!! Form::number('meetings',$target->meetings,['class' => 'form-control', 'placeholder' => trans('admin.meetings'),'min'=>0]) !!}
            </div>
            <div class="form-group @if($errors->has('money')) has-error @endif">
                <label>{{ trans('admin.money') }}</label>
                {!! Form::number('money',$target->money,['class' => 'form-control', 'placeholder' => trans('admin.money'),'min'=>0]) !!}
            </div>
            <div class="form-group @if($errors->has('month')) has-error @endif">
                <label>{{ trans('admin.month') }}</label>
                {!! Form::number('month',$target->month,['class' => 'form-control', 'placeholder' => trans('admin.month'),'min'=>1,'max'=>12]) !!}
            </div>
            <div class="form-group @if($errors->has('notes')) has-error @endif">
                <label>{{ trans('admin.notes') }}</label>
                {!! Form::textarea('notes',$target->notes,['class' => 'form-control', 'placeholder' => trans('admin.notes'),'rows'=>5]) !!}
            </div>
