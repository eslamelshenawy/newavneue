        ...
        <div class="box-body">
            <div class="container">
                ...
                <div class="col-md-6 well">
                    ...
                    {!! Form::open(['url'=>adminPath().'/groups/'.$group->id, 'method'=>'put']) !!}
                        ...
                        <select class="select2 form-control" id="parent_id" name="parent_id"
                            ...
                            @foreach($allCategories as $category)
                                <option value="{{ $category->id }}"
                                        @if($group->parent_id == $category->id) selected @endif @if($group->id == $category->id) disabled @endif>{{ $category->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                        ...
                        {!! Form::text('name', $group->name, ['class'=>'form-control', 'placeholder'=>trans('admin.name')]) !!}
                    </div>

                    <div class="form-group {{ $errors->has('team_leader_id') ? 'has-error' : '' }}">
                        {!! Form::label(trans('admin.team_leader')) !!}
                        <select class="select2 form-control" name="team_leader_id"
                                data-placeholder="{{ trans('admin.team_leader') }}">
                            <option></option>
                            @foreach($agents as $agent)
                                <option value="{{ $agent->id }}"
                                        @if($group->team_leader_id) selected @endif>{{ $agent->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group {{ $errors->has('members') ? 'has-error' : '' }}">
                        ...
                        <select class="select2 form-control" multiple name="members[]"
                                data-placeholder="{{ trans('admin.members') }}">
                            <option></option>
                            @php
                                $members = @App\GroupMember::where('group_id',$group->id)->pluck('member_id')->toArray();
                            @endphp
                            @foreach($agents as $agent)
                                <option value="{{ $agent->id }}"
                                        @if(in_array($agent->id, $members)) selected @endif>{{ $agent->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group {{ $errors->has('notes') ? 'has-error' : '' }}">
                        ...
                        <textarea class="form-control" name="notes"
                                  placeholder="{{ trans('admin.notes') }}">{{ $group->notes }}</textarea>
                    </div>
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function () {
            var id = {{ $group->parent_id }}
            $('#jstree').jstree(true).select_node(id);
        })

        $('#parent_id').on('change', function (e, data) {
            var id = $(this).val();
